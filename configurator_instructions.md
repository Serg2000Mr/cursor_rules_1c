...
| Имя | Тип | Синоним | Подсказка |
|-----|-----|---------|-----------|
| `гк_ДлинаЦепочкиПоставки` | Число(2,0), неотрицательное | Длина цепочки поставки | Длина цепочки поставки |
| `гк_ЦПСогласована` | Булево | ЦП согласована | Цепочка поставки согласована |

> **ТЗ 67470:** при копировании документа реквизиты `гк_ДлинаЦепочкиПоставки` и `гк_ЦПСогласована` должны очищаться.
...
Создать страницу в группе страниц документа:
- Поле **"Статус"** → связать с реквизитом `гк_ЦПСогласована`
- Поле **"Длина цепочки поставки"** → связать с реквизитом `гк_ДлинаЦепочкиПоставки`
- **Таблица** для отображения данных из ТЧ `гк_ЦеныПоставщиков`
...
- Кнопка **«Отобразить данные протокола согласования цен»** — переключает видимость группы колонок «Протокол согласования цен».

#### 3.2.3 Колонки в таблице Товары (при нажатой кнопке)

| Колонка | Назначение |
|---------|------------|
| Группа **«Протокол согласования цен»** | Группирует колонки протокола |
| **«Данные протокола»** | Кнопка выбора → открывает форму `гк_ДанныеПротоколаСогласованияЦен` |
| **«Пр.ЦП»** | Признак проверки цепочки поставки |
...
- [ ] Реквизиты шапки `гк_ДлинаЦепочкиПоставки`, `гк_ЦПСогласована` добавлены
...
- [ ] Поле "Длина цепочки поставки" связано с `гк_ДлинаЦепочкиПоставки`
